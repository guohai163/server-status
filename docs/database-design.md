...
erDiagram
    ...
    NODE_METRIC_SAMPLES ||--o{ GPU_METRIC_SAMPLES : contains
    ...
    GPU_DEVICES ||--o{ GPU_METRIC_SAMPLES : raw
    GPU_DEVICES ||--o{ GPU_METRIC_HOURLY : rollup
    NODES ||--o{ TEMPERATURE_CURRENT_METRICS : latest
    BLOCK_DEVICES ||--o| STORAGE_HEALTH_CURRENT_METRICS : latest
```
...
| 表 | 用途 | 关键约束 |
| --- | --- | --- |
| `nodes` | 节点身份、系统信息、Agent 版本、在线时间和管理员 Tag | `agent_id` 唯一；Tag 最多 5 个 |
...
| `block_devices` | 物理盘、RAID、multipath 或虚拟块设备的型号、容量、协议和 RAID SMART 透传类型 |
...
| `temperature_current_metrics` | CPU、主板、GPU、存储及其他 hwmon 传感器的最新温度和阈值 |
| `storage_health_current_metrics` | 直连盘或 RAID 成员盘的最新 SMART 状态、通电时长、错误计数、磨损与风险 |
...
| 表组 | 粒度 | 分区/保留 |
| --- | --- | --- |
| `*_metric_samples` | 节点、挂载点、网卡或 GPU 每分钟 | UTC 天分区，90 天 |
| `*_current_metrics` | 每个节点或资源一条最新状态 | 不分区，随新样本更新 |
| `*_metric_hourly` | 每节点或资源每小时 | UTC 月分区，24 个月 |

`node_metric_samples` 的主键是 `(bucket_at, node_id)`，也是请求幂等键。它同时保存整机磁盘读写累计值、区间字节数和区间操作数。文件系统、网卡和 GPU 明细通过同一复合键关联主快照，并通过 `(node_id, resource_id)` 复合外键阻止跨节点引用。
...
NVIDIA GPU 由 Agent 调用 `nvidia-smi` 识别，以 GPU UUID 作为稳定清单 key。`gpu_metric_samples` 保存每张 GPU 的分钟核心使用率、显存已用/总量及生成的显存使用率，保留 90 天；其触发器同步 `gpu_current_metrics`。`gpu_metric_hourly` 保存每张 GPU 的小时平均值和最大值，保留 24 个月。无 NVIDIA 驱动或无 GPU 的节点不会生成相关清单和指标。

历史 API 对 `1h`、`6h` 保留分钟点，`24h` 在数据库端按 5 分钟聚合，`7d`、`30d`、`90d` 使用小时汇总。节点、文件系统和网络历史先在限定节点与时间范围内分别聚合，再按时间桶关联，避免对每个节点时间点重复执行资源子查询。成功响应在中央服务内缓存 30 秒，相同节点与范围的并发请求会合并为一次数据库查询。

硬件温度和 SMART 只保存最新快照，不进入分钟分区或小时汇总。SMART 错误数据区分 ATA `Raw_Read_Error_Rate` 的厂商标准化值/原始值，以及 ATA 错误日志、SAS 不可校正错误或 NVMe 介质错误形成的错误计数，避免把不同厂商的原始属性直接当作可比较百分比。风险等级由 SMART 总体结果、smartctl 状态位、待映射/不可校正/重映射扇区、NVMe 严重警告和寿命使用量确定。
...
5. 使用 `INSERT ... ON CONFLICT (bucket_at, node_id) DO UPDATE` 写主快照。重试时先删除该节点、该分钟原有的文件系统、网卡和 GPU 明细，再插入完整明细。
6. 温度与 SMART 以整批 current 快照替换；只有同一分钟重试或更晚报告可以替换，乱序旧报告会被忽略。
7. 在一个数据库事务中提交。四个同步触发器只在新时间不早于 current 表时间时更新最新状态，因此乱序数据不会覆盖较新的状态。
...
- GPU 详情查询：有效 GPU 清单关联 `gpu_current_metrics`，按设备索引逐卡返回；历史查询按设备关联分钟或小时表，逐卡返回核心和显存使用率序列，并与节点历史使用相同时间桶。
...
该函数预建昨天至未来七天的原始分区、上月至未来两个月的小时分区，并删除超过保留期的分区。删除顺序先处理 GPU、文件系统和网络子表，再处理节点主表，满足外键依赖。
...
迁移文件位于 `db/migrations`，必须按版本号顺序执行。`V002` 修正分区清理流程，`V003` 增加磁盘 I/O 原始指标、current 速率和小时汇总，`V004` 增加首页 IP 来源网卡偏好，`V005` 增加 NVIDIA GPU 清单和当前指标，`V006` 增加每节点最多 5 个管理员 Tag，`V007` 增加逐 GPU 分钟历史、小时汇总与 current 同步触发器，`V008` 增加 ARM 性能核与能效核拓扑，`V009` 增加温度与 SMART 当前快照。在连接信息由环境或 `.pgpass` 提供的前提下执行：
...
psql -v ON_ERROR_STOP=1 -f db/migrations/V009__hardware_health_metrics.sql
psql -v ON_ERROR_STOP=1 -f db/verify/V001__verify.sql
```

验收脚本会覆盖节点、令牌、Tag 数量约束、硬件历史、温度/SMART 当前快照、RAID 透传、多 GPU 当前/分钟/小时指标、IPv4/IPv6、首页网卡偏好、current 触发器、幂等更新、跨节点约束、小时汇总、角色权限和查询计划，最后执行 `ROLLBACK`，不会保留验证数据。
