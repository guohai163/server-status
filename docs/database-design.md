...
6. 在一个数据库事务中提交。四个同步触发器只在新时间不早于 current 表时间时更新最新状态，因此乱序数据不会覆盖较新的状态。
...
- GPU 详情查询：有效 GPU 清单关联 `gpu_current_metrics`，按设备索引逐卡返回；历史查询按设备关联分钟或小时表，逐卡返回核心和显存使用率序列。
...
验收脚本会覆盖节点、令牌、Tag 数量约束、硬件历史、IPv4/IPv6、首页网卡偏好、多 GPU 当前/分钟/小时指标、current 触发器、幂等更新、跨节点约束、小时汇总、角色权限和查询计划，最后执行 `ROLLBACK`，不会保留验证数据。
