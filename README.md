...
已有 Windows 节点可在详情页生成无凭据的更新命令；新版 EXE 会复用受 ACL 保护的现有配置，从中心同步 smartmontools，停止服务、替换程序和 SMART 工具并重新启动。
...
| CPU | 厂商、型号、封装数、物理核心、逻辑线程 | 总使用率、1/5/15 分钟 load、温度 |
| 内存 | 插槽、厂商、型号、序列号、类型、容量、速率 | 总量、已用、可用、缓存、buffer、swap |
| 磁盘 | 设备名、厂商、型号、序列号、WWN、介质类型、容量、RAID 透传关系 | 挂载点容量和 inode、整机磁盘读写字节与 IOPS 增量、SMART 温度/通电时长/错误与风险 |
| 网络 | 网卡、MAC、MTU、链路速率、IPv4/IPv6 | 链路状态、收发累计量、区间流量、包、错误和丢包 |
| GPU | NVIDIA 型号、UUID、显存容量 | 每张 GPU 的核心使用率、显存使用率、温度和历史趋势 |
...
2. 从 `/proc`、`/sys` 和系统 API 读取 CPU、内存、块设备、文件系统、网卡与 hwmon 温度；发现 `smartctl` 时额外采集直连盘和 RAID 透传成员盘的 SMART 状态。
...
3. 将节点信息、库存变化、分钟主快照、文件系统、网络、GPU、温度和 SMART 指标放在同一个 PostgreSQL 事务中。
...
```mermaid
flowchart TD
    A["Agent 启动或进入下一分钟"] --> B["采集 CPU、内存、磁盘、温度、SMART、网卡、GPU 和 IP"]
...
- `dist/release/server-status-smartctl-windows-setup.exe`：Windows Agent 自动安装的 smartmontools 7.5 官方安装器。
- `dist/release/server-status-smartctl-source.tar.gz`：与 Windows 安装器对应的 smartmontools 源码。
...
温度从 Linux hwmon 读取，不增加依赖。SMART 与 RAID 控制器透传是可选增强能力，需要目标节点安装 smartmontools 7.0 及以上，并让 Agent 以 root 运行；缺少 `smartctl`、设备不支持 SMART 或控制器/驱动不支持透传时，其他指标仍会正常上报，详情页显示 SMART 未采集。Agent 使用 `smartctl --scan-open --json` 获取每块盘所需的设备类型，因此支持 smartmontools 能识别的 MegaRAID、AACRAID、Areca、3ware、HighPoint、CCISS 和 SSSRAID 透传路径，不需要在 Agent 配置中手工枚举槽位。
...
安装器从中心节点的 Release 接口下载匹配架构的静态二进制和 `checksums.txt`，目标节点不需要访问 GitHub。Windows Agent 还会从同一接口自动同步经过校验的 smartmontools 安装器；首次安装和手工升级都会只提取 `smartctl`、驱动数据库及许可证文档。正式发布的中心镜像直接内置同版本的 Linux `amd64/arm64`、Windows `amd64`、macOS Agent 和 Windows SMART 依赖；`latest` 与镜像固定版本优先读取并校验这些只读资产，不依赖 GitHub 或可写缓存。只有请求历史版本时，中心才从 `guohai163/server-status` 的 GitHub Release 下载、校验并写入持久缓存。Agent 仍会在安装前再次校验 SHA-256，然后原子安装。响应头 `X-Server-Status-Cache` 会分别标记 `BUNDLED`、`HIT` 或 `MISS`。默认路径：
...
| `GET` | `/api/v1/nodes/{node_id}/history?range=24h` | 无 | 查询 `1h/6h/24h/7d/30d/90d` 历史趋势；24h 为 5 分钟、7d 以上为小时分辨率 |
