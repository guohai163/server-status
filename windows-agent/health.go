...
func collectStorageHealth(ctx context.Context, smartctlPath string, inventory []BlockDevice) ([]BlockDevice, []StorageHealth) {
	...
	for index, device := range devices {
		group.Add(1)
		go func(index int, device smartDevice) {
			defer group.Done()
			select {
...
func mergeSMARTSamples(inventory []BlockDevice, samples []smartSample) ([]BlockDevice, []StorageHealth) {
	devices := append([]BlockDevice(nil), inventory...)
	health := make([]StorageHealth, 0, len(samples))
...
func blockDeviceFromSMART(scanned smartDevice, document smartctlDocument) BlockDevice {
	...
	key := firstNonEmpty(document.SerialNumber, document.LogicalUnitID, deviceInfo.Name+"|"+deviceType)
	device := BlockDevice{
		Key: key, DeviceName: firstNonEmpty(deviceInfo.Name, scanned.Name), DeviceKind: "disk",
		Vendor: document.SCSIVendor, ModelName: model, SerialNumber: document.SerialNumber, WWN: document.LogicalUnitID,
		Protocol: firstNonEmpty(deviceInfo.Protocol, scanned.Protocol), SMARTDeviceType: deviceType,
		RAIDPassthrough: isRAIDSMARTType(deviceType),
	}
...
func matchBlockDevice(devices []BlockDevice, candidate BlockDevice) int {
	for index, device := range devices {
...
func mergeBlockDevice(target *BlockDevice, source BlockDevice) {
	if source.Vendor != "" {
...
func storageHealthFromSMART(blockDeviceKey string, document smartctlDocument) StorageHealth {
	metric := StorageHealth{
		BlockDeviceKey: blockDeviceKey, SMARTAvailable: document.SMARTSupport.Available,
		SMARTEnabled: document.SMARTSupport.Enabled, SMARTStatus: "unknown", RiskLevel: "unknown",
	}
